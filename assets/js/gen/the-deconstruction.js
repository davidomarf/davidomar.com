...
const MARGIN_X = 100;        // SVG Margin
const MARGIN_Y = 20;        // SVG Margin
 ...
 */
function initializeCanvas(w, h, color, id) {
    // Create a svgSpace using D3 with custom dimensions
    const svgSpace = d3.select(id)
...
function propagateCluster(center) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        if (
            coordinates[0] < 0 + MARGIN_X
            ||
            coordinates[0] > WIDTH - MARGIN_Y
            ||
            coordinates[1] < 0 + MARGIN_Y
            ||
            coordinates [1] > HEIGHT - MARGIN_Y){
                break;
            }
        fill_solid(coordinates);
 ...
 */
const svgSpace = initializeCanvas(WIDTH, HEIGHT, "#0B0B0B", "#d3-svg-1");
